...
/// with.
///
/// Selecting no attestation-policy-selector uses the administrator-configured defaults, if
/// present, and otherwise leaves the Attestation Service default in place. An
/// unknown attestation-policy-selector is rejected instead of silently falling back, so
/// a client can only reach policies that an administrator declared.
...
pub struct AttestationService {
    ...
    /// Policies selected when a client does not provide an attestation-policy-selector.

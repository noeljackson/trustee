...
3. **Attestation**: The KBC replies to the attestation challenge from step 2
   ...
   key.
4. **Response**: After successful attestation, the KBS returns an `AttestationToken` 
   in the response to the `/kbs/v0/attest` endpoint. Subsequently, the KBC can request 
   resources by sending HTTP `GET` requests to resource specific endpoints. The KBS 
   responds with encrypted resource data in JWE format. Within the valid time of the 
   HTTP Cookie generated by the KBS during step 2, the KBC can directly request 
   resources or services from KBS, without going through steps 2 and 3.
...
advance. An unmapped `attestation-policy-selector` is rejected. When the field is omitted,
...
> **Note:** The `init-data` field is optional. The `tee-pubkey` can be either RSA or EC format (see [Key Format](#key-format) section). The `additional_evidence` is a JSON string representation of a mapping of TEE type to evidence pairs.
...
- `runtime-data`
  ...
  to HW-TEE evidence, ensuring cryptographic integrity and authenticity. It must include:
  - `nonce`: The nonce string from the `Challenge.nonce` received from the KBS in step 2.
...
format and can be either RSA or Elliptic Curve (EC) format, as described in the [key format](#key-format) section.
...
in the KBC's execution environment. The `tee-evidence` is a composite evidence structure containing:
  - `primary_evidence`: Primary TEE evidence. Its format depends on the TEE and is typically defined by the Attestation-Service.
  - `additional_evidence`: Additional evidence for secondary TEE devices within a client. This is a JSON string mapping of TEE type to (TEE class, TEE evidence) pairs, represented as a string to avoid serialization inconsistencies.
...
1. **Attestation Results Token**: Returned by the `/kbs/v0/attest` endpoint upon successful attestation.
2. **Resource Response**: Returned when requesting resources, encrypted using JWE format.
...
The token is a serialized JSON Web Token (JWT) that contains attestation results and endorsements. For detailed information about the token, see the [Attestation Token](../../attestation-service/docs/attestation_token.md) and the [simple token claim in JSON](../../attestation-service/docs/example.token.json).

### Resource Response

Upon successful attestation, the KBC can request resources from the KBS by sending HTTP `GET` requests to it.

If the KBS approves the request, it responds to the KBC by sending a resource response
...
The [`Attestation`](#attestation) `runtime-data.tee-pubkey` field and the [Token Resource](#attestation-results-token)
`tee-pubkey` claim contain a public key from a HW-TEE generated asymmetric key pair.

This field follows the [JSON Web Key](https://www.rfc-editor.org/rfc/rfc7517) format and supports both RSA and Elliptic Curve (EC) key types.
...
content is set to a [Resource Response](#resource-response) JSON payload (JWE format):
...
the [JSON Web Key](https://www.rfc-editor.org/rfc/rfc7517) format and can be either
RSA or Elliptic Curve (EC) format, as described in the [Key Format](#key-format) section.
